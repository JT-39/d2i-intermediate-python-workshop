...
upload = st.file_uploader("Upload 903 data here")
...
if upload:
    ...
    def clean_function():
        for key, df in dfs.items():
            st.write(f"Processing sheet: {key}")
    ...
    # Create measures list output table
    ...
    with st.expander("Download output", expanded=False):
        output_csv = output_table.to_csv(index=False).encode("utf-8")
        st.download_button(
            label="Download processed data as CSV",
            data=output_csv,
            file_name="processed_903.csv"
        )
        st.table(output_table)
    
    # Plot the output table as bar
    ...
    with st.sidebar:
        ...
        # Dynamic measure options based on list selection
        measure_options = output_table[output_table["List"]==list_selected]["Measure"].unique()
    ...
    with st.expander("Sliced plots"):
        ...
        bar = px.bar(
            ...
            labels={"Value": measure_selected, "Count": "Number of Children"}
        )
